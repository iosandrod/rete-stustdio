...
import { Control, InputControl, InsertControl, JSONControl, SelectControl } from './Control'
import { Input, JSONInput, JSONOutput, Output } from './Port'
import { ControlSocket, RefSocket, Socket } from './Socket'
...
export class BaseNode extends ClassicPreset.Node<{ [key in string]: Sockets }, { [key in string]: Sockets }, { [key in string]: Control | InputControl | InsertControl | SelectControl }> {
  ...
  constructor(label: string) {
    super(label)
    this.addInput(BIND_KEY, new Input(new Socket('any'), '', true))
    this.addOutput(BIND_KEY, new Output(new Socket('any'), '', true))
  }
  ...
  updateSize() {
    ...
    this.width = Math.max(labelWidth, 50 + Math.max(...inputsWidths, 0) + Math.max(...outputsWidths, 0))
  }

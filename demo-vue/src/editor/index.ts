...
// UI Components
import Node from './ui/ui/custom-node.vue'
import CustomSocket from './ui/ui/custom-socket.vue'
...
// ============== context-menu.ts ==============
function snippetToItem(snippet: LanguageSnippet, add: (code: string) => unknown | Promise<unknown>): Items<Schemes> {
...
function createItems(snippets: LanguageSnippet[], add: (code: string) => unknown | Promise<unknown>) {
  return (context: any, plugin: any): Items<Schemes> => {
    ...
    const deleteItem: Items<Schemes> = {
      label: 'Delete',
      key: 'delete',
      async handler() {
    ...
    const cloneItem = clone ? {
      label: 'Clone',
      key: 'clone',
      async handler() {
        const node = clone()
        await editor.addNode(node)
        applyInteraction(editor, (id: string) => area.update('node', id))
...
export async function createEditor(
  ...
  const contextMenu = new ContextMenuPlugin<Schemes>({
    items: createItems(snippets, async code => {
  ...
  const innerPortsInstance = useInnerPorts(area, {
    hasLeftPort(node) { return Boolean((node as any).frame?.left) },
    hasRightPort(node) { return Boolean((node as any).frame?.right) },
  ...
  vuePlugin.addPreset(VuePresets.classic.setup({
  ...
  vuePlugin.addPreset(VuePresets.contextMenu.setup({
    delay: 200,
  }))

...
export async function createEditor(
  ...
  domainLayer?: { store: any; bridge: any }
) {
  ...
  connection.addPreset(() => new ClassicFlow({
    makeConnection(from, to, context) {
      ...
      if (source && target) {
        ...
        // Sync with domain layer if provided
        if (domainLayer?.bridge) {
          try {
            domainLayer.bridge.connect(source.nodeId, source.key, target.nodeId, target.key)
          } catch {
            // no-op if domain bridge is not compatible yet
          }
        }
        return true
      }
  ...
  const scopes = new ScopesPlugin<Schemes>({
    exclude: (id) => {
      ...
      return innerPorts.isInnerPort(node)
    },
    size(id, size) {
      const node = editor.getNode(id)
      if (!node) return size
